...
from app.models.models import Guardian, HospitalVisit
...
def list_hospital_visits(
    ...
    """고령층의 병원 방문 일정 목록. 본인 또는 연동 보호자만 조회 가능. active_only=True면 활성 일정만."""
    ...
    return query.order_by(HospitalVisit.visit_datetime.asc()).all()
...
def update_hospital_visit(
    ...
    """병원 방문 일정 수정. 등록한 보호자 본인만 가능."""
    visit = db.query(HospitalVisit).filter(HospitalVisit.visit_id == visit_id).first()
    if visit is None:
        raise HTTPException(status_code=404, detail="병원 방문 일정을 찾을 수 없습니다.")

    if visit.guardian_id != guardian.guardian_id:
        raise HTTPException(status_code=403, detail="본인이 등록한 병원 방문 일정만 수정할 수 있습니다.")
...
def delete_hospital_visit(
    ...
    """병원 방문 일정 삭제. 등록한 보호자 본인만 가능."""
    visit = db.query(HospitalVisit).filter(HospitalVisit.visit_id == visit_id).first()
    if visit is None:
        raise HTTPException(status_code=404, detail="병원 방문 일정을 찾을 수 없습니다.")

    if visit.guardian_id != guardian.guardian_id:
        raise HTTPException(status_code=403, detail="본인이 등록한 병원 방문 일정만 삭제할 수 있습니다.")

    db.delete(visit)
    db.commit()
    return {"status": "success", "message": "병원 방문 일정이 삭제되었습니다."}
