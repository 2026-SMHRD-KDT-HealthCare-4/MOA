...
DEBUG_TEXT_LOGS = os.getenv("MOA_CHAT_DEBUG_TEXT", "").lower() == "true"
...
- Every reply with next_action="continue" MUST include exactly one natural, easy follow-up question.
...
- The question must naturally follow from the user's previous message or gently switch to another daily-life topic.
...
- 사용자가 짧게 답하면 같은 주제를 더 캐묻지 말고, 짧게 받아준 뒤 다른 생활 주제로 부드럽게 전환하세요.
...
- 같은 주제로 1~2번 이상 연속 질문하지 마세요.
- 사용자가 선택·동의·마무리 답변을 하면 그 세부 주제는 마무리하고 다른 생활 주제로 전환하세요.
- 음식 이야기는 메뉴→밥→반찬→국처럼 세부 항목을 연쇄 질문하지 마세요.
- 정보 수집 인터뷰처럼 느껴지지 않게, 한 주제를 마무리하는 짧은 코멘트 뒤 새 주제로 넘어가세요.
- 예/아니오로만 답할 수 있는 질문은 피하세요.
- 사용자의 최신 발화가 명시적으로 대화 종료를 요청하거나 작별 인사를 할 때만 next_action은 "finish"로 설정하세요.
- 대화 턴 수가 충분해 보인다는 이유만으로 먼저 종료하지 마세요.

좋은 흐름 예시:
사용자: 좋아요, 그거 먹을게요
좋음: "제육볶음이면 든든하겠네요. 오늘은 외출 계획도 있으세요?"
나쁨: "그럼 반찬은 뭘 드실 건가요?"
...
def chat_for_frontend(
    ...
    print(f"[USER STT] {_debug_text(message)}")
    forced = detect_rule(message)

    if forced:
        print(f"[INTENT] {forced.get('user_intent')}")
        print(f"[RULE OVERRIDE] applied=true reason={forced.get('override_reason')} reply={_debug_text(forced.get('reply', ''))}")
        print(f"[FINAL BOT REPLY] {_debug_text(forced.get('reply', ''))}")
        print(f"[SOURCE] {forced.get('source', 'rule_override')}")
        return forced
    ...
    topic_instruction = (
        ...
        "If the same topic has already continued for 2 turns, close it briefly and switch to another daily-life topic. "
    ...
    try:
        ...
        print(f"[LLM RAW REPLY] {_debug_text(str(result.get('reply', '')))}")
        ...
        try:
            ...
            print(f"[RULE OVERRIDE] applied=true reason=recovery_empty_llm_content reply={_debug_text(recovery.get('reply', ''))}")
            print(f"[FINAL BOT REPLY] {_debug_text(recovery.get('reply', ''))}")
            print(f"[SOURCE] {recovery.get('source', 'rule_override')}")
            print(f"[CHAT_TIMING] response_parse_ms={round((time.perf_counter() - response_parse_start) * 1000)}")
            return recovery
    ...
    print(f"[INTENT] {validated.get('user_intent')}")
    print(
        "[RULE OVERRIDE] "
        f"applied={validated.get('source') == 'rule_override'} "
        f"reason={validated.get('override_reason')}"
    )
    print(f"[FINAL BOT REPLY] {_debug_text(validated.get('reply', ''))}")
    print(f"[SOURCE] {validated.get('source', 'llm')}")
    print(f"[CHAT_TIMING] response_parse_ms={round((time.perf_counter() - response_parse_start) * 1000)}")
    return validated
