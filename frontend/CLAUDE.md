...
**보호자(가족 탭) 컬러 체계** — 2026-06-23 결정. 4색만 사용: 네이비(CTA `#4F76A8`~`#355A8A`) + 베이지/크림 배경(`#FFF8EF`) + **세이지그린(안정 상태 `#6F9C7A`, 배경 `#EDF5EF`, 상태점 `#7FA38A`)** + 앰버(주의 `#E8943A`). 안정 상태에 채도 높은 병원식 초록 대신 세이지그린을 쓴다. (이전의 "세이지그린 사용하지 않음" 규칙은 보호자 화면 안정 상태색으로 세이지그린을 채택하면서 폐기.) 직접사용자(ELD) 화면은 기존 코랄/웜 톤 유지. 레드는 전 화면 금지.
