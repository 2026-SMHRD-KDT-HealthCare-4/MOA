...
export default function MedicationFormPage() {
  ...
  const resolvedMedicineName = Array.isArray(medicineNameParam)
    ? medicineNameParam[0]
    : medicineNameParam;
  const resolvedCycleType = Array.isArray(cycleTypeParam)
    ? cycleTypeParam[0]
    : cycleTypeParam;
  const resolvedTimes = Array.isArray(timesParam) ? timesParam[0] : timesParam;
  const resolvedStartDate = Array.isArray(startDateParam)
    ? startDateParam[0]
    : startDateParam;
  const resolvedEndDate = Array.isArray(endDateParam)
    ? endDateParam[0]
    : endDateParam;
  ...
  const hasRouteMedicationParams =
    resolvedMedicineName !== undefined ||
    resolvedTimes !== undefined ||
  ...
  const routeTimes = (() => {
    ...
    try {
      ...
      const splitTimes = resolvedTimes
        .split(",")
        .map((time) => time.trim())
        .filter(Boolean);
  ...
  const item = useMedicationStore((s) =>
    s.medications.find((m) => m.id === resolvedId)
  );
  ...
  const [cycle, setCycle] = useState<CycleType>(
    item?.scheduleType ?? item?.cycleType ?? "daily"
  );
  const [days, setDays] = useState<Weekday[]>(item?.daysOfWeek ?? []);
  const [times, setTimes] = useState<string[]>(
    item?.times?.length ? item.times : ["08:00"]
  );
  const [doseMode, setDoseMode] = useState<"1" | "2" | "3" | "custom">(
    item?.times.length === 1
      ? "1"
      : item?.times.length === 2
      ? "2"
  ...
  const setCount = (count: number) =>
    setTimes((current) =>
      Array.from(
        ...
        (_, index) =>
          current[index] ??
          (index === 1 ? "13:00" : index === 2 ? "20:00" : "08:00")
      )
    );

  const chooseDose = (mode: "1" | "2" | "3" | "custom") => {
    setDoseMode(mode);
    if (mode !== "custom") setCount(Number(mode));
  };

  const setSchedule = (next: CycleType) => {
    setCycle(next);
    if (next === "weekly" && days.length !== 1) setDays(["WED"]);
    if (next === "daily") setDays([]);
  };
  ...
  const save = async () => {
    const formattedTimes = times.map((t) => {
      ...
      return /^\d:[0-5]\d$/.test(trimmed) ? "0" + trimmed : trimmed;
    });

    if (
      !name.trim() ||
      !formattedTimes.length ||
      formattedTimes.some((time) => !/^([01]\d|2[0-3]):[0-5]\d$/.test(time))
    ) {
      return Alert.alert(
        "입력 확인",
        "약 이름과 시간을 08:00 형식으로 입력해주세요."
      );
    }
  ...
  const confirmDeleteMedication = (id: string) => {
    ...
    Alert.alert("정말 삭제할까요?", "이 약 정보를 삭제하면 되돌릴 수 없어요.", [
    ...
    <View style={[styles.fill, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {isEditMode ? "약 정보 수정" : "약 추가"}
        </Text>

        {isEditMode ? (
          <TouchableOpacity
            onPress={() => resolvedId && confirmDeleteMedication(resolvedId)}
          >
      ...
      >
        ...
        <Field label="복용 주기">
          <View style={styles.optionRow}>
            {([
              ...
              ["weekly", "주 1회"],
              ["custom_days", "특정 요일"],
            ] as [CycleType, string][]).map(([value, label]) => (
              <TouchableOpacity
                key={value}
                onPress={() => setSchedule(value)}
                style={[styles.option, cycle === value && styles.optionActive]}
              >
                <Text
                  style={[
                    styles.optionText,
                    cycle === value && styles.optionTextActive,
                  ]}
                >
                  {label}
                </Text>
        ...
        {cycle !== "daily" && (
          <Field
            label={
              cycle === "weekly" ? "복용 요일" : "복용할 요일을 모두 선택하세요"
            }
          >
            <View style={styles.dayRow}>
              {WEEKDAYS.map((day) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.dayButton,
                    days.includes(day.key) && styles.dayButtonActive,
                  ]}
                >
                  <Text
                    style={[
                      styles.dayText,
                      days.includes(day.key) && styles.dayTextActive,
                    ]}
                  >
                    {day.label}
                  </Text>
        ...
        <Field label="하루 복용 횟수">
          <View style={styles.doseRow}>
            {([
              ...
              <TouchableOpacity
                key={value}
                onPress={() => chooseDose(value)}
                style={[
                  styles.doseButton,
                  doseMode === value && styles.optionActive,
                ]}
              >
                <Text
                  style={[
                    styles.optionText,
                    doseMode === value && styles.optionTextActive,
                  ]}
                >
                  {label}
                </Text>
        ...
        <Field label="복용 시간">
          {times.map((time, index) => (
            <View key={`time-${index}`} style={styles.timeRow}>
              ...
              {doseMode === "custom" ? (
                ...
                >
                  <X color={times.length === 1 ? "#CFC2BB" : "#A85D46"} />
                </TouchableOpacity>
              ) : null}
            </View>
          ))}
          ...
          {doseMode === "custom" && (
            ...
            >
              <Plus color="#795035" size={19} />
              <Text style={styles.addTimeText}>시간 추가하기</Text>
            </TouchableOpacity>
          )}
        </Field>
        ...
        <View style={styles.noticeRow}>
          <View>
            ...
            <Text style={styles.noticeSub}>설정한 시간에 알림을 보내드려요.</Text>
          </View>

          <Switch
            value={enabled}
            onValueChange={setEnabled}
            trackColor={{ false: "#E5D8D1", true: "#173F73" }}
            thumbColor="white"
          />
        </View>

        <TouchableOpacity onPress={() => void save()} style={styles.saveButton}>
...
const styles = StyleSheet.create({
  ...
  option: {
    ...
    minHeight: 50,
    borderRadius: 14,
    backgroundColor: "rgba(255,249,241,0.8)",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: "rgba(94,65,40,0.12)",
