...
export function useMoaChat() {
  ...
  function endConversationSession(): Promise<void> {
    ...
    const sessionId = useChatSessionStore.getState().sessionId;
    ...
    const request = (async () => {
      try {
        ...
        if (useChatSessionStore.getState().sessionId === sessionId) useChatSessionStore.getState().clearSession();
      } catch (error) {
  ...
  async function sendMessage(text: string, acousticMeta?: Partial<ChatbotApiParams["acoustic_meta"]>) {
    ...
    try {
      const params: ChatbotApiParams = {
        ...
        session_id: useChatSessionStore.getState().sessionId,
      ...
      if (res.data.session_id) useChatSessionStore.getState().setSessionId(res.data.session_id);
