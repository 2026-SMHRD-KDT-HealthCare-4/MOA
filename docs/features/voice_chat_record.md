...
1. 녹음 화면을 열면 `GET /record/script/today`로 오늘의 지정문구를 조회해 낭독 카드에 표시한다. 개발 환경이나 문구 풀이 비어 있는 경우에는 기본 지정문구를 표시한다.
2. 사용자가 녹음 버튼을 누른다.
3. 마이크 권한을 요청한다.
4. 권한이 있으면 호출어 감지를 비활성화한다.
5. `expo-av`로 녹음한다.
6. 사용자가 중지하면 처리 상태로 전환한다.
7. Whisper STT API를 호출하거나 mock transcript를 반환한다.
8. 임시 음성 파일을 삭제한다.
9. 결과 텍스트를 화면에 표시한다.
10. 호출어 감지를 다시 활성화한다.

녹음 화면은 최대 30초를 기준으로 `elapsedSeconds / 30` 진행률을 계산한다. 마이크 버튼 바깥의 SVG 링은 베이지색 배경 위에 초록색 stroke가 0%에서 100%까지 채워지며, 회전 애니메이션을 사용하지 않는다. 30초가 되면 링을 100%로 표시한 뒤 자동으로 녹음을 종료하고 변환 처리로 넘어간다.
...
- 2026-06-22: 녹음 버튼의 회전형 표시를 30초 기준 SVG progress ring으로 교체하고, 완료 시 자동 종료를 연결.
- 2026-06-22: 지정문구 낭독 카드를 오늘의 문구 조회 API와 연결하고 fallback 문구를 추가.
